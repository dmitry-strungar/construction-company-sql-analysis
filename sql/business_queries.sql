...
cte2 as (
    select ep.employee_id, ep.salary * ep.rate as fact
...
select year, total
from (
select extract(year from sign_date) as year, sum(project_cost) as total
from project
...
select sum(avg_stap)
from (
select avg(amount) over (order by fact_transaction_timestamp rows between 2 preceding and 2 following) as avg_stap
from (
select fact_transaction_timestamp, amount, row_number() over (partition by extract(year from fact_transaction_timestamp) order by fact_transaction_timestamp) as rn
from project_payment
